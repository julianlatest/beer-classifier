...
# Find all the subdirectories within the 'tests' folder, extract the dir name as test_dir_parent
for test_dir_parent in $(find "${repo_root}/tests/" -maxdepth 1 -type d | sed "s|${repo_root}/tests/||"); do
    # Check for at least one Python test file in test_dir_parent
    count_test_py_files=$(find "${repo_root}/tests/${test_dir_parent}/" -name "test_*.py" 2>/dev/null | wc -l)
    if [ "$count_test_py_files" -ne 0 ]; then
        # Use the devcontainer Dockerfile to build a Docker image for the module to run tests
        docker build "${repo_root}" -f "${repo_root}/.devcontainer/Dockerfile" -t "${test_dir_parent}"
        ...
        docker run  \
            ...
            /bin/bash -ec '
                ...
                pytest \
                    ...
                    --cov-append \
                    /workspace/tests/$test_dir_parent; \
                exit $?'
    fi
done
